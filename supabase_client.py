...
# Baustein 7: vollautonome Optimierung - KI-Vorschlaege werden sofort aktiv
# geschaltet (kein manuelles Freigeben mehr) und erschoepfte Suchbegriffe
# (mehrere Laeufe in Folge ohne neue Leads) werden automatisch stillgelegt.

from config import FALLBACK_SEARCH_QUERIES, QUERY_EXHAUSTED_AFTER_EMPTY_RUNS
...
def insert_query_suggestions(suggestions: list[dict]) -> int:
    """Schreibt neue KI-Vorschlaege nach Supabase und schaltet sie SOFORT
    aktiv (status='active', source='ai') - Baustein 7: vollautonome
    Optimierung, kein manuelles Freigeben mehr. 'reasoning' bleibt erhalten,
    damit im Dashboard weiterhin nachvollziehbar ist, warum die KI diesen
    Suchbegriff gewaehlt hat. Ueberspringt Duplikate stillschweigend
    (unique-Constraint auf 'query') statt den ganzen Lauf daran scheitern
    zu lassen."""
...
@retry(times=3, delay_seconds=3, exceptions=(Exception,))
def update_query_performance(entries: list[dict]) -> None:
    """Aktualisiert nach jedem Lauf die kumulierten Kennzahlen je aktivem
    Suchbegriff (times_used, total_found/new/warm/mischtyp/kalt) und
    erhoeht/resettet consecutive_empty_runs. Erwartet je Eintrag
    {'query', 'found', 'new', 'warm', 'mischtyp', 'kalt'} - ein Eintrag pro
    in DIESEM Lauf tatsaechlich ausgefuehrtem Suchbegriff, auch wenn er 0
    Treffer hatte (genau das ist das Signal fuer 'erschoepft')."""
    ...
    existing = (
        client.table("search_queries")
        .select(
            "id, query, times_used, total_found, total_new, "
            "total_warm, total_mischtyp, total_kalt, consecutive_empty_runs"
        )
        .eq("status", "active")
        .execute()
    )
    by_query = {row["query"]: row for row in existing.data}

    for e in entries:
        row = by_query.get(e["query"])
        if not row:
            continue  # Suchbegriff wurde inzwischen manuell entfernt - ueberspringen.

        new_count = e.get("new", 0)
        updates = {
            "times_used": row["times_used"] + 1,
            "last_used_at": "now()",
            "total_found": row["total_found"] + e.get("found", 0),
            "total_new": row["total_new"] + new_count,
            "total_warm": row["total_warm"] + e.get("warm", 0),
            "total_mischtyp": row["total_mischtyp"] + e.get("mischtyp", 0),
            "total_kalt": row["total_kalt"] + e.get("kalt", 0),
            "consecutive_empty_runs": (
                0 if new_count > 0 else row["consecutive_empty_runs"] + 1
            ),
        }
        client.table("search_queries").update(updates).eq("id", row["id"]).execute()


@retry(times=2, delay_seconds=3, exceptions=(Exception,))
def retire_exhausted_queries(
    threshold: int = QUERY_EXHAUSTED_AFTER_EMPTY_RUNS,
) -> list[str]:
    """Legt Suchbegriffe automatisch still (status='retired'), die
    {threshold} Laeufe in Folge keine neuen Leads mehr gebracht haben -
    verhindert, dass die Pipeline dieselbe leergesuchte Stadt/Branche
    (z.B. 'Kosmetikstudio Krefeld', nachdem alle dortigen Studios schon
    bekannt sind) endlos weiter abfragt. Gibt die stillgelegten Suchbegriffe
    zurueck, damit main.py sie im Lauf-Log ausgeben kann."""
    ...
    result = (
        ...
        .select("id, query")
        .eq("status", "active")
        .gte("consecutive_empty_runs", threshold)
        .execute()
    )
    retired = [row["query"] for row in result.data]
    for row in result.data:
        client.table("search_queries").update(
            {"status": "retired", "reviewed_at": "now()"}
        ).eq("id", row["id"]).execute()
    return retired


@retry(times=3, delay_seconds=3, exceptions=(Exception,))
def get_query_performance_summary() -> list[dict]:
    """Kumulierte Performance ALLER je verwendeten Suchbegriffe (aktiv UND
    stillgelegt) - Basis fuer datengetriebene KI-Vorschlaege ueber mehrere
    Laeufe hinweg statt nur den letzten (Baustein 7: 'hochprofessionelle
    Parameter' = echte Historie statt Zufallsrauschen eines einzelnen Laufs)."""

    client = _get_client()
    result = (
        client.table("search_queries")
        .select(
            "query, status, times_used, total_found, total_new, "
            "total_warm, total_mischtyp, total_kalt"
        )
